...
export default function StudentDashboard() {
  ...
  const loadStudentData = async () => {
    try {
      ...
      console.log('Stored user data:', { storedName, storedRole, storedEmail, storedUserId });
      ...
      <Box sx={{ p: 3 }}>
        ...
        <Box sx={{ mb: 4, display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box>
            <Typography variant="h4" gutterBottom>
              Welcome back, {student ? `${student.given_name} ${student.family_name}` : 'Student'}!
            </Typography>
            <Typography variant="h6" color="primary" gutterBottom>
              {student ? student.institution_name : 'Your Institution'}
            </Typography>
          </Box>
        </Box>
        ...
        <Paper sx={{ p: 3, mb: 3 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
            ...
            <Typography variant="h5">
              Active Assessments ({assessments.totalActive})
            </Typography>
          </Box>
                ...
                <Card key={assessment.id} sx={{ 
                  ...
                  <CardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
                    ...
                    <Box sx={{ mb: 2 }}>
                      ...
                      <Typography variant="body2" color="text.secondary">
                        <strong>Due:</strong> {formatDate(assessment.available_until)}
                      </Typography>
                    </Box>
        ...
        <Paper sx={{ p: 3 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
            ...
            <Typography variant="h5">
              Completed Assessments ({assessments.totalCompleted})
            </Typography>
          </Box>
                ...
                <Card key={assessment.id} sx={{ 
                  ...
                  <CardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexGrow: 1, mr: 1 }}>
                        ...
                        {isWithinDisputePeriod(assessment) && (
                          <Tooltip title="Within dispute period - you can still dispute this result">
                            <GavelIcon color="warning" fontSize="small" />
                          </Tooltip>
                        )}
                      </Box>
                      <Chip 
                        label={`Score: ${assessment.attempt?.final_grade || 'N/A'}`} 
                        color="success"
                        size="small"
                        sx={{ flexShrink: 0 }}
                      />
                    ...
                    <Box sx={{ mb: 2 }}>
                      ...
                      <Typography variant="body2" color="text.secondary">
                        <strong>Completed:</strong> {assessment.attempt?.completed_at ? formatDate(assessment.attempt.completed_at) : 'N/A'}
                      </Typography>
                      {isWithinDisputePeriod(assessment) && (
                        <Typography variant="body2" color="warning.main" sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                          <WarningIcon fontSize="small" />
                          <strong>Dispute period active</strong>
                        </Typography>
                      )}
                    </Box>
