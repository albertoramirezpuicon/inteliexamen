...
export default function TeacherAttemptsPage() {
  ...
  const handleViewDisputes = (attemptId: number) => {
    router.push(`/teacher/attempts/${attemptId}/disputes`);
  };
  ...
  const handleViewDisputesModal = async (attemptId: number) => {
    ...
    try {
      ...
      const disputesResponse = await fetch(`/api/teacher/disputes/${attemptId}`);
  ...
  const handleSubmitDisputeResponse = async () => {
    ...
    try {
      ...
      if (disputeResponse.ok) {
        ...
        // Refresh disputes
        const refreshResponse = await fetch(`/api/teacher/disputes/${selectedAttemptId}`);
